...
if ($('.oh-card-dashboard').length) {
    steps.push({ element: '#tileContainer .oh-card-dashboard:nth-child(1)', popover: { title: 'Dashboard Tiles', description: 'SU Dashboard Tiles' } });
}
setTimeout(() => {
    ...
    if ($('[data-id="dashboardNav"]').length) {
        steps.push({ element: '[data-id="dashboardNav"]', popover: { title: 'App', description: 'SU Hr Apps. eg Dashboard' } });
    }
}, 1000);
driverObj = driver(
