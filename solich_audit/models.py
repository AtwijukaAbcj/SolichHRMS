...
from solich_audit.methods import remove_duplicate_history
...
class AuditTag(models.Model):
    ...
    class Meta:
        ...
        app_label = "solich_audit"


class SolichAuditInfo(models.Model):
    """
    SolichAuditInfo model to store additional info
    """
...
class SolichAuditLog(HistoricalRecords):
    """
    Model to store additional information for historical records.
    """

    # def __init__(self, *args, bases=None, **kwargs):
    #     super(SolichAuditLog, self).__init__(*args, **kwargs)
    #     self.is_solich_widgets_log = True
...
def pre_create_solich_widgets_log(sender, instance, *args, **kwargs):
    """
    Pre create solich audit log method
    """
    try:
...
def post_create_solich_widgets_log(sender, instance, *_args, **kwargs):
    """
    Post create solich audit log method
    """
    try:
        history_instance = kwargs["history_instance"]
        ...
        if isinstance(history_instance, SolichAuditLog):
