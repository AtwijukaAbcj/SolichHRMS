...
var steps = [
    { popover: { title: 'Dashboard', description: 'Solich dashboard section' } },
...
if ($('.oh-card-dashboard').length) {
    steps.push({ element: '#tileContainer .oh-card-dashboard:nth-child(1)', popover: { title: 'Dashboard Tiles', description: 'Solich Dashboard Tiles' } });
}
setTimeout(() => {
    ...
    if ($('[data-id="dashboardNav"]').length) {
        steps.push({ element: '[data-id="dashboardNav"]', popover: { title: 'App', description: 'Solich Hr Apps. eg Dashboard' } });
    }
}, 1000);
driverObj = driver(
